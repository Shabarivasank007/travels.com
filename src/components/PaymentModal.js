...
const PaymentModal = ({ package: pkg, bookingData, onClose, onPaymentSuccess }) => {
  ...
  const handleRazorpayPayment = () => {
    ...
    const totalAmount = parseInt(pkg.price.replace('₹', '').replace(',', '')) * parseInt(bookingData.travelers);
          ...
          <div className="bg-gradient-to-r from-blue-50 to-purple-50 p-6 rounded-xl mb-6 border border-blue-200">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span>Traveler: {bookingData.name}</span>
                <span>Date: {bookingData.date}</span>
              </div>
              <div className="flex justify-between">
                <span>Travelers: {bookingData.travelers}</span>
                <span>Per person: {pkg.price}</span>
              </div>
